...
--
-- organizers and customers were removed (see NOTES.md "Removing
-- organizers and customers") -- organizer_id/customer_id were removed as
-- bare columns too, not just their descriptive-attribute tables, since
-- neither is read by any of the four report requirements. This file is
-- additive-only (IF NOT EXISTS), like the rest of init_db.py -- on a
-- database created before this change, run `make reset && make init-db`
-- to get a clean schema rather than trying to migrate column-by-column.
...
GO
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_raw_partnership_agreements_reseller_id')
CREATE INDEX IX_raw_partnership_agreements_reseller_id ON raw_b2b.partnership_agreements(reseller_id);
GO
